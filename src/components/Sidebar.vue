...
const SideBar = [
    { id: 1, name: "Organization", icon: Organization, activeIcon: OrganizationActive },
    ...
    { id: 8, name: "ScanHistory", icon: ScanHistory, activeIcon: ScanHistoryActive },
];
...
<template>
    <div :class="[$style.container, sideBarStatus ? '' : $style.small]">
        ...
        <div :class="$style.sideBarButtonContainer">
            <CloseSideBar :class="[$style.closeSideBar, !sideBarStatus ? $style.rotated : '']" @click="toggleSidebar" />
        </div>
        <div :class="$style.sideBarContainer">
            <div v-for="item in SideBar" :key="item.id" @click="selectedSideBar = item.id" :class="[
                $style.sideBarItem,
                selectedSideBar === item.id ? $style.selected : ''
            ]" style="display: flex; align-items: center; cursor: pointer;">
                <component :is="selectedSideBar === item.id ? item.activeIcon : item.icon" />

                <span v-if="sideBarStatus" :class="$style.sideBarName">{{ item.name }}</span>
            </div>
        </div>
    </div>
</template>
